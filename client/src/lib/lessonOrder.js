 ...
 *
 * The client keeps its own copy because a lesson just uploaded is merged into
 * the list the screen already holds rather than re-fetched — appending would
 * put it last here and somewhere else after the next reload. It has to stay in
 * step with server/src/lib/lessonOrder.js.
 */
...
/** A copy of the list in lesson order — never a sort in place on React state. */
export function sortedLessons(modules) {
